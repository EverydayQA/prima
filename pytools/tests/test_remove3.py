...
import unittest
import mock
from other.fsample.remove3 import RemovalService
from other.fsample.remove3 import UploadService
from other.fsample.read import Foo
...
class TestRemovalService(unittest.TestCase):
    ...
    # patch 2 sub
    @mock.patch('other.fsample.remove3.os.path')
    @mock.patch('other.fsample.remove3.os')
    ...
    @mock.patch('other.fsample.remove3.glob')
...
class TestMockOpen(unittest.TestCase):
    ...
    def test_mock_open(self):
        ...
        """
        with mock.patch("other.fsample.read.open",
                        mock.mock_open(read_data=TEST_DATA), create=True):
            # read an empty input file
            fee = Foo(self.input)
            # got data that is not empty? with mocking?
    ...
    def test_mock_open_class(self):
        ...
        """
        with mock.patch("other.fsample.read.Foo.open",
                        mock.mock_open(read_data=TEST_DATA), create=True):
            fee = Foo(self.input)
            self.assertEqual(fee.data, self.expected)
